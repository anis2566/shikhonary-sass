...
# Add libc6-compat for cases where Next.js or its dependencies need it (especially on Alpine)
RUN apk add --no-cache libc6-compat
WORKDIR /app

# Optimization: Small, secure, production-ready environment
ENV NODE_ENV=production
ENV NEXT_TELEMETRY_DISABLED=1
ENV PORT=3000
...
# Standard Next.js Standalone mode output includes all required node_modules
COPY --from=builder --chown=nextjs:nodejs /app/apps/admin/.next/standalone ./
# Static files and public assets are not bundled in standalone mode for performance reasons (usually served by a CDN/Proxy)
# But for a single-container VPS setup, we serve them via the Next.js server
...
# Note: Next.js standalone entrypoint is always apps/[app_name]/server.js
