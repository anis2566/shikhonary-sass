...
import { ACADEMIC_LEVEL } from "@workspace/utils";

import { useCreateAcademicClass } from "@workspace/api-client";

export function ClassForm() {
  const router = useRouter();
  const { mutateAsync: createClass, isPending } = useCreateAcademicClass();
    ...
    <div className="max-w-4xl mx-auto p-4 lg:p-6 space-y-8 animate-in fade-in duration-500 text-foreground">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-muted-foreground font-medium">
              Start by defining a new academic class
            </p>
          </div>
        </div>
      </div>
      ...
      <Card className="bg-card/30 backdrop-blur-xl border-border/50 rounded-[2rem] overflow-hidden shadow-medium relative">
        ...
        <CardHeader className="pb-2">
          <CardTitle className="text-xl font-bold">New Class Details</CardTitle>
