...
export function EditChapterForm({ chapterId }: EditChapterFormProps) {
  ...
  const { mutateAsync: updateChapter, isPending } = useUpdateAcademicChapter();
  ...
  useEffect(() => {
    if (chapter) {
      ...
      const dbSubjectId = (chapter.subjectId as string) || "";
      const normalizedSubjectId = dbSubjectId.trim();
  ...
  const onSubmit = async (data: AcademicChapterFormValues) => {
    try {
      await updateChapter({ id: chapterId, data });
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-muted-foreground font-medium">
              Refine the details of your academic chapter
            </p>
          </div>
        </div>
      </div>
      ...
      <Card className="bg-card/30 backdrop-blur-xl border-border/50 rounded-[2rem] overflow-hidden shadow-medium relative">
        ...
        <CardHeader className="pb-2">
          ...
          <CardDescription className="text-muted-foreground font-medium">
            Fill in the details below to update the chapter.
          </CardDescription>
        </CardHeader>

        <CardContent className="pt-6">
          <Form {...form} key={chapter?.id}>
