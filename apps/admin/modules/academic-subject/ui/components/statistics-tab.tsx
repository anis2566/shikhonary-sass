...
import { useAcademicSubjectStatistics } from "@workspace/api-client";

interface StatisticsTabProps {
  subjectId: string;
}

export const StatisticsTab = ({ subjectId }: StatisticsTabProps) => {
  const { data: statsData, isLoading } =
    useAcademicSubjectStatistics(subjectId);
  ...
  const { contentDistribution, hierarchy, questionBank } = stats;
        ...
        <Card className="bg-card/50 backdrop-blur-md border-border/50 rounded-3xl overflow-hidden shadow-soft">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <CardDescription className="text-[10px] font-bold uppercase tracking-widest opacity-70">
                  Topic weightage across chapters
                </CardDescription>
              </div>
            </div>
          </CardHeader>
          <CardContent className="p-8 space-y-6">
            {contentDistribution.length === 0 ? (
              ...
              contentDistribution.map((chapter) => (
                <div key={chapter.id} className="space-y-2 group">
                  <div className="flex justify-between items-end">
                    ...
                    <span className="text-[10px] font-black uppercase tracking-widest text-muted-foreground/60 bg-muted/50 px-2 py-0.5 rounded-lg border border-border/50">
                      {chapter.topics} topics ({chapter.percentage}%)
                    </span>
                  </div>
                  <Progress
          ...
          <CardHeader className="pb-6 bg-muted/30 border-b border-border/50">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <CardDescription className="text-[10px] font-bold uppercase tracking-widest opacity-70">
                  Vertical breakdown of curriculum
                </CardDescription>
              </div>
            </div>
          </CardHeader>
          <CardContent className="p-8">
            <div className="flex flex-wrap items-center justify-between gap-6 overflow-x-auto py-2">
              <div className="text-center flex-1 min-w-[120px] p-4 rounded-2xl bg-background/50 border border-border/50 shadow-soft hover:shadow-glow transition-all group">
                <div className="w-12 h-12 rounded-2xl bg-yellow-500/10 flex items-center justify-center mx-auto mb-3 border border-yellow-500/20 group-hover:scale-110 transition-transform">
              ...
              <ChevronRight className="h-6 w-6 text-muted-foreground/30 shrink-0 hidden sm:block" />

              <div className="text-center flex-1 min-w-[120px] p-4 rounded-2xl bg-background/50 border border-border/50 shadow-soft hover:shadow-glow transition-all group">
                <div className="w-12 h-12 rounded-2xl bg-purple-500/10 flex items-center justify-center mx-auto mb-3 border border-purple-500/20 group-hover:scale-110 transition-transform">
              ...
              <div className="text-center flex-1 min-w-[120px] p-4 rounded-2xl bg-background/50 border border-border/50 shadow-soft hover:shadow-glow transition-all group">
                <div className="w-12 h-12 rounded-2xl bg-blue-500/10 flex items-center justify-center mx-auto mb-3 border border-blue-500/20 group-hover:scale-110 transition-transform">
              ...
              <div className="text-center flex-1 min-w-[120px] p-4 rounded-2xl bg-primary/5 border border-primary/10 shadow-soft hover:shadow-glow transition-all group">
                <div className="w-12 h-12 rounded-2xl bg-primary/10 flex items-center justify-center mx-auto mb-3 border border-primary/20 group-hover:scale-110 transition-transform">
