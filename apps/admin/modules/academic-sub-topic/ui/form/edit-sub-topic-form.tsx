"use client";
import { ChevronLeft, Loader2, Save, Sparkles, Layers } from "lucide-react";
...
import {
  ...
  AcademicSubTopicFormValues,
  defaultAcademicSubTopicValues,
} from "@workspace/schema";
import {
  useAcademicSubTopicById,
  useUpdateAcademicSubTopic,
  useAcademicClassesForSelection,
  useAcademicSubjectByClassId,
  useAcademicChaptersForSelection,
  useAcademicTopicByChapterId,
...
export function EditSubTopicForm({ subTopicId }: EditSubTopicFormProps) {
  const router = useRouter();
  const { data: subTopic } = useAcademicSubTopicById(subTopicId);
  const { mutateAsync: updateSubTopic, isPending } =
    useUpdateAcademicSubTopic();

  const { data: classes } = useAcademicClassesForSelection();

  const form = useForm<AcademicSubTopicFormValues>({
    resolver: zodResolver(academicSubTopicFormSchema),
    defaultValues: defaultAcademicSubTopicValues,
  });
  ...
  const { data: topics } = useAcademicTopicByChapterId(selectedChapterId || "");
  ...
  useEffect(() => {
    if (subTopic) {
      form.reset({
        ...
        classId: subTopic.topic?.chapter?.subject?.class?.id,
  ...
  const onSubmit = async (data: AcademicSubTopicFormValues) => {
    try {
      await updateSubTopic({ id: subTopicId, data });
      router.push("/sub-topics");
    } catch (error) {
      console.error(error);
    }
  };
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-muted-foreground font-medium">
              Update the details of your academic sub-topic
            </p>
          </div>
        </div>
      </div>
                ...
                render={({ field }) => (
                  <FormItem className="flex flex-row items-center justify-between rounded-[1.5rem] border border-border/50 bg-primary/5 p-6 shadow-soft transition-all hover:bg-primary/[0.07]">
                    <div className="space-y-1">
                      ...
                      <CardDescription className="text-muted-foreground font-medium">
                        Publish this sub-topic to make it visible to users.
                      </CardDescription>
                    </div>
                    <FormControl>
                      <Switch
