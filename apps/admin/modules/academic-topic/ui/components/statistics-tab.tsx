...
import { useAcademicTopicStatistics } from "@workspace/api-client";

interface StatisticsTabProps {
  topicId: string;
}

export const StatisticsTab = ({ topicId }: StatisticsTabProps) => {
  const { data, isLoading } = useAcademicTopicStatistics(topicId);
      ...
      <TabsContent value="statistics" className="mt-0 space-y-6">
        <div className="text-center py-20 bg-card/50 backdrop-blur-md rounded-3xl border border-border/50 shadow-soft">
          ...
          <p className="text-lg font-black text-foreground">
            Failed to load topic statistics
          </p>
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <CardDescription className="text-[10px] font-bold uppercase tracking-widest opacity-70">
                  Breakdown of content across sub-topics
                </CardDescription>
              </div>
            </div>
          </CardHeader>
          <CardContent className="p-8 space-y-6">
            {data.contentDistribution.length === 0 ? (
              <p className="text-sm text-muted-foreground text-center py-8 font-medium">
                No sub-topics data available
              </p>
            ) : (
              data.contentDistribution.map((subTopic) => (
                <div key={subTopic.id} className="space-y-2 group">
                  <div className="flex justify-between items-end">
                    ...
                    <span className="text-[10px] font-black uppercase tracking-widest text-muted-foreground/60 bg-muted/50 px-2 py-0.5 rounded-lg border border-border/50">
                      {subTopic.questions} q ({subTopic.percentage}%)
                    </span>
                  </div>
                  <Progress
