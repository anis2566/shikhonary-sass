...
export const McqForm = () => {
  ...
  const { mutateAsync: createMCQ, isPending } = useCreateMCQ();
  ...
  useEffect(() => {
    if (subjectIdParam) {
      form.setValue("subjectId", subjectIdParam, { shouldValidate: true });
    }
    if (chapterIdParam) {
      form.setValue("chapterId", chapterIdParam, { shouldValidate: true });
    }
  }, [subjectIdParam, chapterIdParam, form]);

  // Reset chapter/topic when subject changes
  useEffect(() => {
        ...
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem className="space-y-2">
                      ...
                      >
                        <FormControl>
                          <SelectTrigger className="h-12 bg-background/50 border-border/50 rounded-xl px-4 font-semibold w-full">
          ...
          <Card className="bg-card/30 backdrop-blur-xl border-border/50 rounded-[2rem] overflow-hidden shadow-medium">
            <CardHeader className="pb-2">
              ...
              <CardDescription className="text-muted-foreground font-medium">
                Add at least 2 options and specify the correct answer
              </CardDescription>
            </CardHeader>
                ...
                render={({ field }) => (
                  <FormItem className="space-y-2">
                    ...
                    <p className="text-xs text-muted-foreground font-medium">
                      Type the exact option text that is correct, or the option
                      letter (A, B, C, D...)
                    </p>
                    <FormMessage className="font-bold text-xs" />
                  </FormItem>
